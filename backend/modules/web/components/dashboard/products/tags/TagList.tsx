...
export const TagList: React.FC<TagListProps> = ({ tags, onDelete, onEdit }) => {
            ...
            {tags.length === 0 ? (
              ...
              tags.map((tag) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-500 dark:text-gray-400">{formatDate(tag.updatedAt)}</span>
                  </td>
                  <td className="px-6 py-4 text-right action-menu-container">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (activeMenu?.id === tag.id) {
                          setActiveMenu(null);
                        } else {
