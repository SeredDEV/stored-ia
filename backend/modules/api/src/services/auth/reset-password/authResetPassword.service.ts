  ...
  async confirmPasswordReset(
    ...
    if (result.error) {
      ...
      if (
        ...
        if (this.errorDictionaryGenerator) {
          const validationName = errorMessage.includes("expired")
            ? "tokenExpired"
            : "invalidToken";
          const error = await this.errorDictionaryGenerator.create({
            validationName,
        ...
        throw new InvalidCredentialsError(
          "El token de restablecimiento es inválido o ha expirado. Por favor solicita un nuevo enlace."
        );
      }

      throw new InvalidCredentialsError(
        result.error.message || "No se pudo actualizar la contraseña"
      );
    }
